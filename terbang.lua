-- [[ ZONHUB - FLY MODULE (STANDALONE FIX) ]] --
local TargetPage = ... 
if not TargetPage then warn("Module harus di-load dari ZonIndex!") return end

getgenv().ScriptVersion = "FlyModule v2.0 - Standalone Fix" 
...
local function CreateSlider(Parent, Text, Min, Max, Default, Var)
    ...
    getgenv()[Var] = Default
    -- Logika slider dapat disesuaikan dengan UI library ZonHub Anda
end
...
-- ========================================== --
-- LOGIKA TERBANG (STABIL & BISA BUILD)
-- ========================================== --
...
RunService.Heartbeat:Connect(function()
    ...
    if getgenv().FlyEnabled and Root and Hum then
        ...
        -- Mengunci rotasi agar tetap tegak
        BodyGyro.CFrame = Root.CFrame
        
        -- Mengikuti arah MoveDirection (WASD)
        BodyVel.Velocity = Hum.MoveDirection * getgenv().FlySpeed
        
        -- Jika diam, berikan sedikit tenaga angkat agar tidak meluncur turun
        if Hum.MoveDirection.Magnitude == 0 then
            BodyVel.Velocity = Vector3.new(0, 0.1, 0)
        end
        
        -- PlatformStand membuat karakter bisa terbang tanpa animasi jatuh
